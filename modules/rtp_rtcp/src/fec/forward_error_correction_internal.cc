/*
 * forward_error_correction_internal.cc - FEC internal implementation
 *
 * Copyright (c) 2024 The WebRTC project authors
 * Copyright (c) 2024 Aspect Software
 *
 * This file is derived from WebRTC's forward_error_correction_internal.cc
 */
...
#include "media/modules/rtp_rtcp/src/fec/fec_private_tables_random.h"
...
std::span<const uint8_t> PacketMaskTable::LookUp(int num_media_packets,
  ...
  AVE_DCHECK_LE(num_fec_packets, num_media_packets);
...
size_t PacketMaskSize(size_t num_sequence_numbers) {
  AVE_DCHECK_LE(num_sequence_numbers, 8 * kUlpfecPacketMaskSizeLBitSet);
