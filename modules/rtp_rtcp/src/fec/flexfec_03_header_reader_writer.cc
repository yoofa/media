/*
 * flexfec_03_header_reader_writer.cc - FlexFEC 03 header reader/writer impl
 *
 * Copyright (c) 2016 The WebRTC project authors. All Rights Reserved.
 * Ported to aspect-video-engine by aspect.
 */

#include "media/modules/rtp_rtcp/src/fec/flexfec_03_header_reader_writer.h"
#include <span>
...
bool Flexfec03HeaderReader::ReadFecHeader(
  ...
  if (fec_packet->pkt->data.size() <=
      kBaseHeaderSize + kStreamSpecificHeaderSize) {
    AVE_LOG(LS_WARNING) << "Discarding truncated FlexFEC packet.";
    return false;
  }
  ...
  if (fec_packet->pkt->data.size() < kHeaderSizes[0]) {
    AVE_LOG(LS_WARNING) << "Discarding truncated FlexFEC03 packet.";
    return false;
  }
...
size_t Flexfec03HeaderWriter::MinPacketMaskSize(const uint8_t* packet_mask,
  ...
  AVE_DCHECK(false) << "Incorrect packet mask size: " << packet_mask_size
                    << ".";
  return kFlexfecPacketMaskSizes[2];
}
...
void Flexfec03HeaderWriter::FinalizeFecHeader(
  ...
  AVE_CHECK(protected_streams.size() == 1);
