...
void UlpfecGenerator::SetProtectionParameters(
  ...
  AVE_DCHECK_GE(delta_params.fec_rate, 0);
  AVE_DCHECK_LE(delta_params.fec_rate, 255);
  AVE_DCHECK_GE(key_params.fec_rate, 0);
...
void UlpfecGenerator::AddPacketAndGenerateFec(const RtpPacketToSend& packet) {
  ...
  if (media_packets_.size() < kUlpfecMaxMediaPackets) {
    ...
    AVE_DCHECK_GE(packet.headers_size(), kRtpHeaderSize);
...
std::vector<std::unique_ptr<RtpPacketToSend>> UlpfecGenerator::GetFecPackets() {
  ...
  AVE_CHECK(last_media_packet_.has_value());
