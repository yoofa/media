...
class ArrayOfArrayViews {
 ...
 private:
  using Storage = std::vector<std::span<const uint8_t>>;
...
using VectorObuInfo = std::vector<ObuInfo>;
...
VectorObuInfo ParseObus(
  ...
  if (expect_continues_obu) {
    AVE_LOG(LS_WARNING) << "Last packet shouldn't have last obu fragmented.";
    return {};
  }
  return obu_infos;
}
...
bool CalculateObuSizes(ObuInfo* obu_info) {
  if (obu_info->data.empty()) {
    AVE_LOG(LS_WARNING) << "Invalid bitstream: empty obu provided.";
    return false;
  }
  ...
  if (!ObuHasSize(obu_header)) {
    ...
    if (obu_size_bytes != obu_info->payload_size) {
      ...
      AVE_LOG(LS_WARNING) << "Mismatch in obu_size. signaled: "
                          << obu_size_bytes
  ...
  obu_info->prefix_size +=
      WriteLeb128(base::dchecked_cast<uint64_t>(obu_info->payload_size),
...
std::shared_ptr<EncodedImageBuffer> VideoRtpDepacketizerAv1::AssembleFrame(
    std::span<const std::span<const uint8_t>> rtp_payloads) {
  ...
  std::shared_ptr<EncodedImageBuffer> bitstream =
  ...
  AVE_CHECK_EQ(write_at - bitstream->data(),
               static_cast<ptrdiff_t>(bitstream->size()));
  return bitstream;
}
