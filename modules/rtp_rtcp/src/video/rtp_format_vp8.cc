...
bool ValidateHeader(const RTPVideoHeaderVP8& hdr_info) {
  if (hdr_info.pictureId != kNoPictureId) {
    AVE_DCHECK_GE(hdr_info.pictureId, 0);
    AVE_DCHECK_LE(hdr_info.pictureId, 0x7FFF);
  }
  if (hdr_info.tl0PicIdx != kNoTl0PicIdx) {
    AVE_DCHECK_GE(hdr_info.tl0PicIdx, 0);
    AVE_DCHECK_LE(hdr_info.tl0PicIdx, 0xFF);
  }
  if (hdr_info.temporalIdx != kNoTemporalIdx) {
    AVE_DCHECK_GE(hdr_info.temporalIdx, 0);
    AVE_DCHECK_LE(hdr_info.temporalIdx, 3);
  } else {
    AVE_DCHECK(!hdr_info.layerSync);
  }
  if (hdr_info.keyIdx != kNoKeyIdx) {
    AVE_DCHECK_GE(hdr_info.keyIdx, 0);
    AVE_DCHECK_LE(hdr_info.keyIdx, 0x1F);
  }
  return true;
}
...
RtpPacketizerVp8::RawHeader RtpPacketizerVp8::BuildHeader(
  ...
  AVE_DCHECK(ValidateHeader(header));
