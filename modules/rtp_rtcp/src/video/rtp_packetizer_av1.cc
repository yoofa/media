...
std::vector<RtpPacketizerAv1::Obu> RtpPacketizerAv1::ParseObus(
  ...
  base::ByteBufferReader payload_reader(payload);
  while (payload_reader.Length() > 0) {
    ...
    if (ObuHasExtension(obu.header)) {
      if (payload_reader.Length() == 0) {
        AVE_LOG(LS_ERROR) << "Malformed AV1 input: expected extension_header, "
    ...
    if (!ObuHasSize(obu.header)) {
      ...
      if (!payload_reader.ReadUVarint(&size) ||
        ...
        AVE_LOG(LS_ERROR) << "Malformed AV1 input: declared size " << size
...
std::vector<RtpPacketizerAv1::Packet> RtpPacketizerAv1::Packetize(
  ...
  if (limits.max_payload_len - limits.last_packet_reduction_len < 3 ||
    ...
    AVE_LOG(LS_ERROR) << "Failed to packetize AV1 frame: requested packet "
  ...
  for (size_t obu_index = 0; obu_index < obus.size(); ++obu_index) {
    ...
    if (is_last_obu &&
      ...
      packets.emplace_back(/*first_obu_index=*/obu_index);
      Packet& semi_last_packet = packets.back();
      semi_last_packet.num_obu_elements = 1;
      semi_last_packet.first_obu_offset = obu_offset;
      semi_last_packet.last_obu_size = semi_last_fragment_size;
      semi_last_packet.packet_size = semi_last_fragment_size;
...
std::vector<RtpPacketizerAv1::Packet> RtpPacketizerAv1::PacketizeAboutEqually(
  ...
  if (packet_size_left_unused >
    ...
    AVE_DCHECK_GT(limits.max_payload_len, size_reduction);
    AVE_DCHECK_GT(size_reduction, kBytesOverheadEvenDistribution);
    ...
    AVE_LOG(LS_WARNING) << "AV1 even distribution caused a regression in "
...
bool RtpPacketizerAv1::NextPacket(RtpPacketToSend* packet) {
  ...
  AVE_DCHECK_GT(next_packet.num_obu_elements, 0);
  AVE_DCHECK_LT(next_packet.first_obu_offset,
  ...
  *write_at++ = AggregationHeader();
  ...
  AVE_DCHECK_EQ(write_at - rtp_payload,
                kAggregationHeaderSize + next_packet.packet_size);
