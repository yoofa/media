/*
 * video_rtp_depacketizer_h265.cc
...
#include "media/modules/rtp_rtcp/src/video/rtp_packet_h265_common.h"
...
std::optional<VideoRtpDepacketizer::ParsedRtpPayload> ProcessApOrSingleNalu(
  ...
  if (rtp_payload.size() < kH265PayloadHeaderSizeBytes) {
    AVE_LOG(LS_ERROR) << "RTP payload truncated.";
    return std::nullopt;
  }
  ...
  parsed_payload->video_header.codec = VideoCodecType::kVideoCodecH265;
  ...
  if (nal_type == H265::NaluType::kAp) {
    ...
    if (rtp_payload.size() <= kH265ApHeaderSizeBytes) {
      AVE_LOG(LS_ERROR) << "Aggregated packet header truncated.";
      return std::nullopt;
    }
  ...
  for (size_t i = 0; i < nalu_start_offsets.size() - 1; ++i) {
    ...
    if (end_offset - start_offset < kH265NalHeaderSizeBytes) {
      AVE_LOG(LS_ERROR) << "Aggregated packet too short";
      return std::nullopt;
    }

    // Insert start code before each NALU in aggregated packet.
    video_payload.AppendData(kH265StartCode);
    ...
    switch (nalu_type) {
      ...
      case H265::NaluType::kSps: {
        ...
        if (sps) {
          ...
          AVE_LOG(LS_WARNING) << "Failed to parse SPS from SPS slice.";
        }
      }
        [[fallthrough]];
      ...
      case H265::NaluType::kPaci:
        AVE_LOG(LS_WARNING) << "Unexpected AP, FU or PACI received.";
        return std::nullopt;
    }
...
std::optional<VideoRtpDepacketizer::ParsedRtpPayload> ParseFuNalu(
  ...
  if (rtp_payload.size() < kH265FuHeaderSizeBytes + kH265NalHeaderSizeBytes) {
    AVE_LOG(LS_ERROR) << "FU NAL units truncated.";
    return std::nullopt;
  }
  std::optional<VideoRtpDepacketizer::ParsedRtpPayload> parsed_payload(
      std::in_place);
...
VideoRtpDepacketizerH265::Parse(base::CopyOnWriteBuffer rtp_payload) {
