...
size_t SsDataLength(const RTPVideoHeaderVP9& hdr) {
  ...
  AVE_CHECK_LE(hdr.gof.num_frames_in_gof, kMaxVp9FramesInGof);
  size_t length = 1;  // V
  ...
  for (size_t i = 0; i < hdr.gof.num_frames_in_gof; ++i) {
    AVE_CHECK_LE(hdr.gof.num_ref_pics[i], kMaxVp9RefPics);
...
//      +-+-+-+-+-+-+-+-+              -|           -|
//
bool WriteSsData(const RTPVideoHeaderVP9& vp9, base::BitBufferWriter* writer) {
  AVE_CHECK_GT(vp9.num_spatial_layers, 0U);
  AVE_CHECK_LE(vp9.num_spatial_layers, kMaxVp9NumberOfSpatialLayers);
  AVE_CHECK_LE(vp9.gof.num_frames_in_gof, kMaxVp9FramesInGof);
...
RtpPacketizerVp9::RtpPacketizerVp9(std::span<const uint8_t> payload,
  ...
  AVE_CHECK_EQ(hdr_.first_active_layer, 0);
...
bool RtpPacketizerVp9::NextPacket(RtpPacketToSend* packet) {
  ...
  // dropped.
  AVE_CHECK(hdr_.spatial_idx < hdr_.num_spatial_layers - 1 ||
            hdr_.end_of_picture);
...
bool RtpPacketizerVp9::WriteHeader(bool layer_begin,
  ...
  base::BitBufferWriter writer(buffer.data(), buffer.size());
  ...
  if (i_bit && !WritePictureId(hdr_, &writer)) {
    AVE_LOG(LS_ERROR) << "Failed writing VP9 picture id.";
    return false;
  }
  if (l_bit && !WriteLayerInfo(hdr_, &writer)) {
    AVE_LOG(LS_ERROR) << "Failed writing VP9 layer info.";
    return false;
  }
  if (p_bit && f_bit && !WriteRefIndices(hdr_, &writer)) {
    AVE_LOG(LS_ERROR) << "Failed writing VP9 ref indices.";
    return false;
  }
  if (v_bit && !WriteSsData(hdr_, &writer)) {
    AVE_LOG(LS_ERROR) << "Failed writing VP9 SS data.";
    return false;
  }
  ...
  AVE_DCHECK_EQ(offset_bits, 0);
  AVE_DCHECK_EQ(offset_bytes, buffer.size());
  return true;
}
