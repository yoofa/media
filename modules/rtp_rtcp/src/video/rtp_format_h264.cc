...
RtpPacketizerH264::RtpPacketizerH264(std::span<const uint8_t> payload,
  ...
  AVE_CHECK(packetization_mode == H264PacketizationMode::NonInterleaved ||
...
void RtpPacketizerH264::NextAggregatePacket(RtpPacketToSend* rtp_packet) {
  ...
  AVE_CHECK_GE(payload_capacity, kNalHeaderSize);
  uint8_t* buffer = rtp_packet->AllocatePayload(payload_capacity);
  AVE_DCHECK(buffer);
  PacketUnit* packet = &packets_.front();
  AVE_CHECK(packet->first_fragment);
  // STAP-A NALU header.
  buffer[0] =
  ...
  while (packet->aggregated) {
    ...
    AVE_CHECK_LE(index + kLengthFieldSize + fragment.size(), payload_capacity);
