/*
 * rtp_to_ntp_estimator.cc - RTP to NTP timestamp estimator implementation
 *
...
RtpToNtpEstimator::UpdateResult RtpToNtpEstimator::UpdateMeasurements(
  ...
  if (invalid_sample) {
    ...
    AVE_LOG(LS_WARNING) << "Multiple consecutively invalid RTCP SR reports, "
...
NtpTime RtpToNtpEstimator::Estimate(uint32_t rtp_timestamp) const {
  ...
  return NtpTime(base::saturated_cast<uint64_t>(estimated));
}
