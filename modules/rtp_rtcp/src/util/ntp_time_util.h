...
#ifndef AVE_MODULES_RTP_RTCP_SOURCE_NTP_TIME_UTIL_H_
#define AVE_MODULES_RTP_RTCP_SOURCE_NTP_TIME_UTIL_H_
...
// Overload for base::NtpTime
inline uint32_t CompactNtp(base::NtpTime ntp) {
  return (ntp.seconds() << 16) | (ntp.fractions() >> 16);
}
...
inline constexpr int64_t ToNtpUnits(base::TimeDelta delta) {
  ...
  return (base::saturated_cast<int32_t>(delta.us()) * (int64_t{1} << 32)) /
         1'000'000;
}
...
#endif  // AVE_MODULES_RTP_RTCP_SOURCE_NTP_TIME_UTIL_H_
