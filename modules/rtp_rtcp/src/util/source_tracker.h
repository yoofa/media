...
#ifndef AVE_MODULES_RTP_RTCP_SOURCE_SOURCE_TRACKER_H_
#define AVE_MODULES_RTP_RTCP_SOURCE_SOURCE_TRACKER_H_
...
#include "media/modules/rtp_rtcp/src/util/rtp_source.h"
...
class SourceTracker {
 public:
  ...
  static constexpr base::TimeDelta kTimeout = base::TimeDelta::Seconds(10);

  explicit SourceTracker(base::Clock* clock);
  ...
  void OnFrameDelivered(
      ...
      base::Timestamp delivery_time = base::Timestamp::MinusInfinity());
 ...
 private:
  ...
  struct SourceEntry {
    ...
    base::Timestamp timestamp = base::Timestamp::MinusInfinity();
    ...
    std::optional<base::TimeDelta> local_capture_clock_offset;
  ...
  // pruning in getters.
  void PruneEntries(base::Timestamp now) const;
...
#endif  // AVE_MODULES_RTP_RTCP_SOURCE_SOURCE_TRACKER_H_
