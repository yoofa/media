/*
 * receive_statistics_impl.cc - RTP receive statistics implementation
 * Ported from WebRTC (modules/rtp_rtcp/source/receive_statistics_impl.cc)
 *
...
#include "media/modules/rtp_rtcp/src/util/receive_statistics_impl.h"
...
constexpr int64_t kNtpJan1970Millisecs = 2208988800000LL;
...
void StreamStatisticianImpl::UpdateCounters(const RtpPacketReceived& packet) {
  AVE_DCHECK_EQ(ssrc_, packet.Ssrc());
...
void StreamStatisticianImpl::MaybeAppendReportBlockAndReset(
  ...
  AVE_DCHECK_GE(exp_since_last, 0);
  ...
  if (packets_lost > 0x7fffff) {
    ...
    if (!cumulative_loss_is_capped_) {
      ...
      AVE_LOG(LS_WARNING) << "Cumulative loss reached maximum value for ssrc "
                          << ssrc_;
    }
...
std::vector<rtcp::ReportBlock> ReceiveStatisticsImpl::RtcpReportBlocks(
  ...
  for (size_t i = 0; i < all_ssrcs_.size() && result.size() < max_blocks; ++i) {
    ...
    AVE_DCHECK(statistician_it != statisticians_.end());
