...
#ifndef AVE_MODULES_RTP_RTCP_SOURCE_RTCP_PACKET_TRANSPORT_FEEDBACK_H_
#define AVE_MODULES_RTP_RTCP_SOURCE_RTCP_PACKET_TRANSPORT_FEEDBACK_H_
...
class TransportFeedback : public Rtpfb {
 public:
  class ReceivedPacket {
   public:
    ...
    base::TimeDelta delta() const { return delta_ticks_ * kDeltaTick; }
  ...
  static constexpr base::TimeDelta kDeltaTick = base::TimeDelta::Micros(250);
  ...
  void SetBase(
      ...
      base::Timestamp ref_timestamp);  // Reference timestamp for this msg.
  ...
  bool AddReceivedPacket(uint16_t sequence_number, base::Timestamp timestamp);
  const std::vector<ReceivedPacket>& GetReceivedPackets() const;

  // Calls `handler` for all packets this feedback describes.
  // For received packets pass receive time as `delta_since_base` since the
  // `BaseTime()`. For missed packets calls `handler` with `delta_since_base =
  // PlusInfinity()`.
  void ForAllPackets(
      std::function<void(uint16_t sequence_number,
                         base::TimeDelta delta_since_base)> handler) const;
  ...
  base::Timestamp BaseTime() const;

  // Get the unwrapped delta between current base time and `prev_timestamp`.
  base::TimeDelta GetBaseDelta(base::Timestamp prev_timestamp) const;
 ...
 private:
  ...
  base::Timestamp last_timestamp_;
...
#endif  // AVE_MODULES_RTP_RTCP_SOURCE_RTCP_PACKET_TRANSPORT_FEEDBACK_H_
