...
bool Remb::Parse(const CommonHeader& packet) {
  AVE_DCHECK(packet.type() == kPacketType);
  AVE_DCHECK_EQ(packet.fmt(), Psfb::kAfbMessageType);

  if (packet.payload_size_bytes() < 16) {
    AVE_LOG(LS_WARNING) << "Payload length " << packet.payload_size_bytes()
  ...
  if (packet.payload_size_bytes() !=
    ...
    AVE_LOG(LS_WARNING) << "Payload size " << packet.payload_size_bytes()
...
bool Remb::SetSsrcs(std::vector<uint32_t> ssrcs) {
  if (ssrcs.size() > kMaxNumberOfSsrcs) {
    AVE_LOG(LS_WARNING) << "Not enough space for all given SSRCs.";
    return false;
  }
...
bool Remb::Create(uint8_t* packet,
  ...
  CreateHeader(Psfb::kAfbMessageType, kPacketType, HeaderLength(), packet,
               index);
  AVE_DCHECK_EQ(0u, Psfb::media_ssrc());
