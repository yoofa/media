...
bool Bye::Parse(const CommonHeader& packet) {
  ...
  if (has_reason) {
    ...
    if (packet.payload_size_bytes() - 4u * src_count < 1u + reason_length) {
      AVE_LOG(LS_WARNING) << "Invalid reason length: " << reason_length;
      return false;
    }
  }
...
bool Bye::SetCsrcs(std::vector<uint32_t> csrcs) {
  if (csrcs.size() > static_cast<size_t>(kMaxNumberOfCsrcs)) {
    AVE_LOG(LS_WARNING) << "Too many CSRCs for Bye packet.";
    return false;
  }
...
void Bye::SetReason(std::string_view reason) {
  AVE_DCHECK_LE(reason.size(), 0xffu);
