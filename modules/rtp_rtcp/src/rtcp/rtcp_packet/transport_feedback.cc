...
constexpr base::TimeDelta kBaseTimeTick =
    TransportFeedback::kDeltaTick * (1 << 8);
constexpr base::TimeDelta kTimeWrapPeriod = kBaseTimeTick * (1 << 24);
...
uint16_t TransportFeedback::LastChunk::Emit() {
  AVE_DCHECK(!CanAdd(0) || !CanAdd(1) || !CanAdd(2));
  if (all_same_) {
  ...
  AVE_DCHECK_GE(size_, kMaxTwoBitCapacity);
...
void TransportFeedback::LastChunk::DecodeOneBit(uint16_t chunk,
                                                size_t max_size) {
  AVE_DCHECK_EQ(chunk & 0xc000, 0x8000);
...
void TransportFeedback::LastChunk::DecodeTwoBit(uint16_t chunk,
                                                size_t max_size) {
  AVE_DCHECK_EQ(chunk & 0xc000, 0xc000);
...
TransportFeedback::TransportFeedback(bool include_timestamps)
    : base_seq_no_(0),
      ...
      last_timestamp_(base::Timestamp::Zero()),
...
bool TransportFeedback::AddReceivedPacket(uint16_t sequence_number,
  ...
  if (include_timestamps_) {
    ...
    AVE_DCHECK_GE(timestamp, last_timestamp_);
    int64_t delta_full =
...
void TransportFeedback::ForAllPackets(
    std::function<void(uint16_t, base::TimeDelta)> handler) const {
  base::TimeDelta delta_since_base = base::TimeDelta::Zero();
  ...
  for (uint16_t seq_num = base_seq_no_; seq_num != last_seq_num; ++seq_num) {
    if (received_it != received_packets_.end() &&
      ...
      handler(seq_num, base::TimeDelta::PlusInfinity());
    }
  }
  AVE_DCHECK(received_it == received_packets_.end());
}
...
base::Timestamp TransportFeedback::BaseTime() const {
  ...
  return base::Timestamp::Zero() + kTimeWrapPeriod +
         int64_t{base_time_ticks_} * kBaseTimeTick;
}

base::TimeDelta TransportFeedback::GetBaseDelta(
    base::Timestamp prev_timestamp) const {
  base::TimeDelta delta = BaseTime() - prev_timestamp;
...
bool TransportFeedback::Parse(const CommonHeader& packet) {
  ...
  if (packet.payload_size_bytes() < kMinPayloadSizeBytes) {
    AVE_LOG(LS_WARNING) << "Buffer too small (" << packet.payload_size_bytes()
                        << " bytes) to fit a "
  ...
  if (status_count == 0) {
    AVE_LOG(LS_WARNING) << "Empty feedback messages not allowed.";
    return false;
  }
  ...
  while (delta_sizes.size() < status_count) {
    if (index + kChunkSizeBytes > end_index) {
      AVE_LOG(LS_WARNING) << "Buffer overflow while parsing packet.";
      Clear();
      return false;
    }
  ...
  AVE_DCHECK_EQ(delta_sizes.size(), status_count);
  num_seq_no_ = status_count;

  uint16_t seq_no = base_seq_no_;
  size_t recv_delta_size =
      std::accumulate(delta_sizes.begin(), delta_sizes.end(), size_t{0});
  ...
  if (end_index >= index + recv_delta_size) {
    for (size_t delta_size : delta_sizes) {
      AVE_DCHECK_LE(index + delta_size, end_index);
      switch (delta_size) {
        case 0:
          break;
        case 1: {
        ...
        case 3:
          Clear();
          AVE_LOG(LS_WARNING) << "Invalid delta_size for seq_no " << seq_no;
...
bool TransportFeedback::IsConsistent() const {
  ...
  if (num_seq_no_ != delta_sizes.size()) {
    AVE_LOG(LS_ERROR) << delta_sizes.size() << " packets encoded. Expected "
                      << num_seq_no_;
    return false;
  }
  base::Timestamp timestamp = BaseTime();
  auto packet_it = received_packets_.begin();
  uint16_t seq_no = base_seq_no_;
  for (DeltaSize delta_size : delta_sizes) {
    if (delta_size > 0) {
      if (packet_it == received_packets_.end()) {
        AVE_LOG(LS_ERROR) << "Failed to find delta for seq_no " << seq_no;
        return false;
      }
      if (packet_it->sequence_number() != seq_no) {
        AVE_LOG(LS_ERROR) << "Expected to find delta for seq_no " << seq_no
                          << ". Next delta is for "
      ...
      if (delta_size == 1 &&
        ...
        AVE_LOG(LS_ERROR) << "Delta " << packet_it->delta_ticks()
                          << " for seq_no " << seq_no
  ...
  if (packet_it != received_packets_.end()) {
    AVE_LOG(LS_ERROR) << "Unencoded delta for seq_no "
                      << packet_it->sequence_number();
    return false;
  }
  if (timestamp != last_timestamp_) {
    AVE_LOG(LS_ERROR) << "Last timestamp mismatch. Calculated: " << timestamp
                      << ". Saved: " << last_timestamp_;
    return false;
  }
  if (size_bytes_ != packet_size) {
    AVE_LOG(LS_ERROR) << "Rtcp packet size mismatch. Calculated: "
