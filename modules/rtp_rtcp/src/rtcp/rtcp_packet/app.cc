...
bool App::Parse(const CommonHeader& packet) {
  ...
  if (packet.payload_size_bytes() < kAppBaseLength) {
    AVE_LOG(LS_WARNING) << "Packet is too small to be a valid APP packet";
    return false;
  }
...
void App::SetSubType(uint8_t subtype) {
  AVE_DCHECK_LE(subtype, 0x1f);
  sub_type_ = subtype;
}

void App::SetData(const uint8_t* data, size_t data_length) {
  AVE_DCHECK(data);
  AVE_DCHECK_EQ(data_length % 4, 0u) << "Data must be 32 bits aligned.";
  AVE_DCHECK_LE(data_length, kMaxDataSize)
