...
#ifndef MEDIA_MODULES_RTP_RTCP_SOURCE_RTCP_RECEIVER_H_
#define MEDIA_MODULES_RTP_RTCP_SOURCE_RTCP_RECEIVER_H_
...
class RTCPReceiver final {
 public:
  ...
  RTCPReceiver(const RTCPReceiver&) = delete;
  RTCPReceiver& operator=(const RTCPReceiver&) = delete;
  ...
  // has not received RTP packets for too long.
 ...
 private:
  ...
  class RegisteredSsrcs {
   public:
    ...
    RegisteredSsrcs(const RtpRtcpInterface::Configuration& config);
  ...
  struct LastFirStatus {
    LastFirStatus() : request(Timestamp::Zero()), sequence_number(0) {}
  ...
  mutable std::mutex rtcp_receiver_lock_;
  ...
  std::map<uint32_t, NonSenderRttStats> non_sender_rtts_;
...
#endif  // MEDIA_MODULES_RTP_RTCP_SOURCE_RTCP_RECEIVER_H_
