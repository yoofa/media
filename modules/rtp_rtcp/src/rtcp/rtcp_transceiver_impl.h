...
class RtcpTransceiverImpl {
 public:
  ...
  AVE_DISALLOW_COPY_AND_ASSIGN(RtcpTransceiverImpl);
 ...
 private:
  ...
  // Use std::map instead of flat_map
  std::map<uint32_t, RemoteSenderState> remote_senders_;
  std::list<LocalSenderState> local_senders_;
  std::map<uint32_t, std::list<LocalSenderState>::iterator>
      local_senders_by_ssrc_;
  std::map<uint32_t, RrtrTimes> received_rrtrs_;
  base::RepeatingTaskHandle periodic_task_handle_;
};
