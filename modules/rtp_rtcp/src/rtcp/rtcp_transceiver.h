...
class RtcpTransceiver : public RtcpFeedbackSenderInterface {
 public:
  ...
  AVE_DISALLOW_COPY_AND_ASSIGN(RtcpTransceiver);
  ...
  // Calls to observer will be done on the `config.task_runner`.
  ...
  void ReceivePacket(base::CopyOnWriteBuffer packet);
 ...
 private:
  ...
  base::TaskRunnerBase* const task_runner_;
