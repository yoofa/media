...
#include "media/modules/rtp_rtcp/src/rtcp/rtcp_packet/compound_packet.h"
...
#include "media/modules/rtp_rtcp/src/rtcp/rtcp_packet/rrtr.h"
...
#include "media/modules/rtp_rtcp/src/rtcp/rtcp_packet/target_bitrate.h"
...
RTCPSender::RTCPSender(base::Clock* clock, Configuration config)
    : clock_(clock),
      audio_(config.audio),
  ...
  AVE_DCHECK(transport_ != nullptr);
  AVE_DCHECK(clock_ != nullptr);
...
void RTCPSender::SetRemb(int64_t bitrate_bps, std::vector<uint32_t> ssrcs) {
  ...
  // Send a REMB immediately if we have a new REMB.
...
void RTCPSender::BuildSR(const RtcpContext& ctx, PacketSender& sender) {
  ...
  report.SetNtp(ToNtpTime(clock_->ConvertTimestampToNtpTime(ctx.now_)));
...
void RTCPSender::BuildSDES(const RtcpContext& /* ctx */, PacketSender& sender) {
  ...
  AVE_CHECK_LT(length_cname, kRtcpCnameSize);
...
void RTCPSender::BuildExtendedReports(const RtcpContext& ctx,
  ...
  if (!sending_ && xr_send_receiver_reference_time_enabled_) {
    rtcp::Rrtr rrtr;
    rrtr.SetNtp(ToNtpTime(clock_->ConvertTimestampToNtpTime(ctx.now_)));
    xr.SetRrtr(rrtr);
  }
...
std::optional<int32_t> RTCPSender::ComputeCompoundRTCPPacket(
  ...
  while (it != report_flags_.end()) {
    ...
    if (builder_it == builders_.end()) {
      AVE_DCHECK(false) << "Could not find builder for packet type "
                        << rtcp_packet_type;
    } else {
  ...
  AVE_DCHECK(AllVolatileFlagsConsumed());
  return std::nullopt;
}
...
void RTCPSender::PrepareReport(const FeedbackState& feedback_state) {
  ...
  if (generate_report) {
    ...
    AVE_DCHECK(!(IsFlagPresent(kRtcpSr) && IsFlagPresent(kRtcpRr)));
  }
}
...
void RTCPSender::SetVideoBitrateAllocation(
  ...
  if (new_bitrate) {
    ...
    AVE_LOG(LS_INFO) << "Emitting TargetBitrate XR for SSRC " << ssrc_
...
void RTCPSender::SendCombinedRtcpPacket(
  ...
  compound.Build(max_packet_size, callback);
}
