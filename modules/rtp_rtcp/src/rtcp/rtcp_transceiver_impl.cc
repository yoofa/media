...
#include "media/modules/rtp_rtcp/src/rtcp/rtcp_transceiver_impl.h"
#include <algorithm>
#include <span>
...
// Convert base::NtpTime to ave::NtpTime (media NtpTime)
inline NtpTime ToMediaNtpTime(const base::NtpTime& base_ntp) {
...
std::function<void(std::span<const uint8_t>)> GetRtcpTransport(
  ...
  return [first, log_prefix](std::span<const uint8_t> /* packet */) mutable {
    if (first) {
      AVE_LOG(LS_ERROR) << log_prefix << "Sending RTCP packets is disabled.";
      first = false;
    }
  };
}
...
struct RtcpTransceiverImpl::LocalSenderState {
  ...
  std::map<uint32_t, uint8_t> last_fir;
...
class RtcpTransceiverImpl::PacketSender {
 public:
  PacketSender(rtcp::RtcpPacket::PacketReadyCallback callback,
    ...
    AVE_DCHECK_LE(max_packet_size, IP_PACKET_SIZE);
  }
  ~PacketSender() { AVE_DCHECK_EQ(index_, 0u) << "Unsent rtcp packet."; }
...
void RtcpTransceiverImpl::RemoveMediaReceiverRtcpObserver(
  ...
  auto it = std::ranges::find(stored, observer);
  if (it == stored.end()) {
    return;
  }
  stored.erase(it);
...
void RtcpTransceiverImpl::ReceivePacket(std::span<const uint8_t> packet,
  ...
  if (!report_blocks.empty()) {
    ProcessReportBlocks(now, std::span<const ReportBlockData>(
                                 report_blocks.data(), report_blocks.size()));
  }
}
...
void RtcpTransceiverImpl::HandleSenderReport(
  ...
  const auto& blocks = sender_report.report_blocks();
  HandleReportBlocks(
      sender_report.sender_ssrc(), now,
      std::span<const rtcp::ReportBlock>(blocks.data(), blocks.size()),
      report_blocks);
...
void RtcpTransceiverImpl::HandleReceiverReport(
  ...
  const auto& blocks = receiver_report.report_blocks();
  HandleReportBlocks(
      receiver_report.sender_ssrc(), now,
      std::span<const rtcp::ReportBlock>(blocks.data(), blocks.size()),
      report_blocks);
}
...
void RtcpTransceiverImpl::HandleReportBlocks(
  ...
  base::NtpTime base_now_ntp = config_.clock->ConvertTimestampToNtpTime(now);
  NtpTime now_ntp = ToMediaNtpTime(base_now_ntp);
  uint32_t receive_time_ntp = CompactNtp(now_ntp);
  Timestamp now_utc = base::Clock::NtpToUtc(base_now_ntp);
...
void RtcpTransceiverImpl::HandleNack(
  ...
  if (it != local_senders_by_ssrc_.end()) {
    const auto& packet_ids = nack.packet_ids();
    it->second->handler->OnNack(
        nack.sender_ssrc(),
        std::span<const uint16_t>(packet_ids.data(), packet_ids.size()));
  }
}
...
void RtcpTransceiverImpl::HandleTransportFeedback(
    const rtcp::CommonHeader& rtcp_packet_header,
    Timestamp now) {
  AVE_DCHECK_EQ(rtcp_packet_header.fmt(),
...
void RtcpTransceiverImpl::HandleExtendedReports(
  ...
  if (config_.reply_to_non_sender_rtt_measurement && extended_reports.rrtr()) {
    ...
    rrtr.local_receive_mid_ntp_time = CompactNtp(
        ToMediaNtpTime(config_.clock->ConvertTimestampToNtpTime(now)));
  }
...
void RtcpTransceiverImpl::HandleDlrr(const rtcp::Dlrr& dlrr, Timestamp now) {
  ...
  uint32_t receive_time_ntp =
      CompactNtp(ToMediaNtpTime(config_.clock->ConvertTimestampToNtpTime(now)));
...
void RtcpTransceiverImpl::ProcessReportBlocks(
  ...
  AVE_DCHECK(!report_blocks.empty());
  if (config_.network_link_observer == nullptr) {
    return;
  }
...
void RtcpTransceiverImpl::SchedulePeriodicCompoundPackets(TimeDelta delay) {
  ...
  periodic_task_handle_ = base::RepeatingTaskHandle::DelayedStart(
      config_.task_runner, delay.us(), [this]() -> uint64_t {
        AVE_DCHECK(config_.schedule_periodic_compound_packets);
        AVE_DCHECK(ready_to_send_);
        SendPeriodicCompoundPacket();
        return config_.report_period.us();
      });
}
...
std::vector<uint32_t> RtcpTransceiverImpl::FillReports(
  ...
  AVE_DCHECK(rtcp_sender.IsEmpty());

  size_t available_bytes = config_.max_packet_size;
  if (reserved.per_packet > available_bytes) {
    // Because reserved.per_packet is uint32_t, substracting would underflow and
  ...
  for (auto it = local_senders_.begin();
    ...
    if (stats.num_sent_bytes() < rtp_sender.last_num_sent_bytes) {
      AVE_LOG(LS_ERROR) << "Inconsistent SR for SSRC " << rtp_sender.ssrc
    ...
    sender_report.SetNtp(
        ToMediaNtpTime(config_.clock->ConvertTimestampToNtpTime(now)));
    AVE_DCHECK_GE(now, stats.last_capture_time());
  ...
  size_t num_receiver_reports = base::DivideRoundUp(
      static_cast<size_t>(report_blocks.end() - report_block_it),
  ...
  AVE_DCHECK_EQ(report_blocks.end() - report_block_it, 0);
  return sender_ssrcs;
}
...
void RtcpTransceiverImpl::CreateCompoundPacket(Timestamp now,
  ...
  if (!config_.cname.empty()) {
    ...
    AVE_DCHECK(added) << "Failed to add CNAME " << config_.cname
                      << " to RTCP SDES packet.";
  ...
  if (!received_rrtrs_.empty()) {
    AVE_DCHECK(config_.reply_to_non_sender_rtt_measurement);
    xr_with_dlrr.emplace();
    uint32_t now_ntp = CompactNtp(
        ToMediaNtpTime(config_.clock->ConvertTimestampToNtpTime(now)));
  ...
  if (!has_sender_report && config_.non_sender_rtt_measurement) {
    ...
    rtcp::Rrtr rrtr;
    rrtr.SetNtp(ToMediaNtpTime(config_.clock->ConvertTimestampToNtpTime(now)));
  ...
  if (xr_with_dlrr.has_value()) {
    ...
    if (config_.reply_to_non_sender_rtt_mesaurments_on_all_ssrcs &&
        !sender_ssrcs.empty()) {
      ssrcs =
          std::span<const uint32_t>(sender_ssrcs.data(), sender_ssrcs.size());
    }
    AVE_DCHECK(!ssrcs.empty());
