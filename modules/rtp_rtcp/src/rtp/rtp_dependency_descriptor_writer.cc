...
NextLayerIdc GetNextLayerIdc(const FrameDependencyTemplate& previous,
  ...
  AVE_DCHECK_LT(next.spatial_id, DependencyDescriptor::kMaxSpatialIds);
  AVE_DCHECK_LT(next.temporal_id, DependencyDescriptor::kMaxTemporalIds);
...
RtpDependencyDescriptorWriter::RtpDependencyDescriptorWriter(
  ...
  if (base::SafeNe(descriptor.frame_dependencies.chain_diffs.size(),
...
void RtpDependencyDescriptorWriter::WriteTemplateDependencyStructure() {
  AVE_DCHECK_GE(structure_.structure_id, 0);
  AVE_DCHECK_LT(structure_.structure_id, DependencyDescriptor::kMaxTemplates);
  AVE_DCHECK_GT(structure_.num_decode_targets, 0);
  AVE_DCHECK_LE(structure_.num_decode_targets,
...
void RtpDependencyDescriptorWriter::WriteTemplateLayers() {
  ...
  AVE_DCHECK(!templates.empty());
  AVE_DCHECK_LE(templates.size(),
                static_cast<size_t>(DependencyDescriptor::kMaxTemplates));
  AVE_DCHECK_EQ(templates[0].spatial_id, 0);
...
void RtpDependencyDescriptorWriter::WriteTemplateDtis() {
  for (const FrameDependencyTemplate& current_template : structure_.templates) {
    AVE_DCHECK_EQ(current_template.decode_target_indications.size(),
...
void RtpDependencyDescriptorWriter::WriteTemplateChains() {
  AVE_DCHECK_GE(structure_.num_chains, 0);
...
void RtpDependencyDescriptorWriter::WriteResolutions() {
  ...
  for (const RenderResolution& resolution : structure_.resolutions) {
    AVE_DCHECK_GT(resolution.Width(), 0);
    AVE_DCHECK_LE(resolution.Width(), 1 << 16);
    AVE_DCHECK_GT(resolution.Height(), 0);
    AVE_DCHECK_LE(resolution.Height(), 1 << 16);
...
void RtpDependencyDescriptorWriter::WriteFrameDtis() {
  AVE_DCHECK_EQ(descriptor_.frame_dependencies.decode_target_indications.size(),
                static_cast<size_t>(structure_.num_decode_targets));
  for (DecodeTargetIndication dti :
       descriptor_.frame_dependencies.decode_target_indications) {
    WriteBits(static_cast<uint32_t>(dti), 2);
