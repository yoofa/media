...
#ifndef AVE_MEDIA_MODULES_RTP_RTCP_SOURCE_RTP_PACKET_TO_SEND_H_
#define AVE_MEDIA_MODULES_RTP_RTCP_SOURCE_RTP_PACKET_TO_SEND_H_
...
// The metadata is not sent over the wire, but packet sender may use it to
...
class RtpPacketToSend : public RtpPacket {
 public:
  ...
  base::Timestamp capture_time() const { return capture_time_; }
  void set_capture_time(base::Timestamp time) { capture_time_ = time; }
  ...
  void set_time_in_send_queue(base::TimeDelta time_in_send_queue) {
    time_in_send_queue_ = time_in_send_queue;
  }
  std::optional<base::TimeDelta> time_in_send_queue() const {
    return time_in_send_queue_;
  }

  // Sets the packetization finish time in the VideoTimingExtension.
  // The timestamp is converted to a delta from capture_time and written
  // to the extension data.
  void set_packetization_finish_time(base::Timestamp time);
 ...
 private:
  base::Timestamp capture_time_ = base::Timestamp::Zero();
  ...
  bool is_red_ = false;
  std::optional<base::TimeDelta> time_in_send_queue_;
};
...
#endif  // AVE_MEDIA_MODULES_RTP_RTCP_SOURCE_RTP_PACKET_TO_SEND_H_
