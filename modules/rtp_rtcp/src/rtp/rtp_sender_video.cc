/*
 * rtp_sender_video.cc - RTP Video Sender Implementation
 *
...
#include "media/modules/rtp_rtcp/src/rtp/rtp_sender_video.h"
...
bool IsBaseLayer(const RTPVideoHeader& video_header) {
  ...
  if (video_header.generic) {
    ...
    return (generic.temporal_index == 0 ||
            std::cmp_equal(generic.temporal_index, kNoTemporalIdx));
  }
...
bool PacketWillLikelyBeRequestedForRestransmissionIfLost(
  ...
  return IsBaseLayer(video_header) &&
         !(video_header.generic.has_value()
               ? std::find(
                     video_header.generic->decode_target_indications.begin(),
                     video_header.generic->decode_target_indications.end(),
...
void RTPSenderVideo::SetVideoStructureInternal(
  ...
  AVE_DCHECK_GT(video_structure->num_decode_targets, 0);
  AVE_DCHECK_GT(video_structure->templates.size(), 0);
...
void RTPSenderVideo::AddRtpHeaderExtensions(const RTPVideoHeader& video_header,
  ...
  if (video_header.generic) {
    ...
    if (packet->IsRegistered<RtpDependencyDescriptorExtension>() &&
      ...
      // Convert std::vector to absl::InlinedVector
      descriptor.frame_dependencies.chain_diffs.assign(
          video_header.generic->chain_diffs.begin(),
          video_header.generic->chain_diffs.end());
      descriptor.frame_dependencies.decode_target_indications.assign(
          video_header.generic->decode_target_indications.begin(),
          video_header.generic->decode_target_indications.end());
      AVE_DCHECK_EQ(
      ...
      if (video_header.frame_type == VideoFrameType::kVideoFrameKey &&
        ...
        descriptor.attached_structure =
            std::unique_ptr<FrameDependencyStructure>(video_structure_.get());
      }
...
bool RTPSenderVideo::SendVideo(int32_t payload_type,
  ...
  if (video_structure_ != nullptr && video_header.generic) {
    // Convert absl::InlinedVector and std::vector to std::span by providing
    // data() and size()
    active_decode_targets_tracker_.OnFrame(
        std::span<const int32_t>(
            video_structure_->decode_target_protected_by_chain.data(),
            video_structure_->decode_target_protected_by_chain.size()),
        video_header.generic->active_decode_targets,
        video_header.frame_type == VideoFrameType::kVideoFrameKey,
        video_header.generic->frame_id,
        std::span<const int32_t>(video_header.generic->chain_diffs.data(),
                                 video_header.generic->chain_diffs.size()));
  }
  ...
  int32_t packet_capacity = rtp_sender_->MaxRtpPacketSize();
  if (use_fec) {
  ...
  std::unique_ptr<RtpPacketToSend> single_packet = rtp_sender_->AllocatePacket(
      std::span<const uint32_t>(csrcs.data(), csrcs.size()));
  AVE_DCHECK_LE(packet_capacity,
                static_cast<int32_t>(single_packet->capacity()));
  ...
  if (video_header.absolute_capture_time.has_value()) {
    video_header.absolute_capture_time =
        absolute_capture_time_sender_.OnSendPacket(
            AbsoluteCaptureTimeSender::GetSource(
                single_packet->Ssrc(),
                std::span<const uint32_t>(csrcs.data(), csrcs.size())),
  ...
  if (video_structure_ != nullptr &&
    ...
    AVE_DCHECK_EQ(video_header.frame_type, VideoFrameType::kVideoFrameKey);
    ...
    AVE_LOG(LS_WARNING) << "Disable dependency descriptor because failed to "
  ...
  AVE_DCHECK_GT(packet_capacity,
                static_cast<int32_t>(single_packet->headers_size()));
  AVE_DCHECK_GT(packet_capacity,
                static_cast<int32_t>(first_packet->headers_size()));
  AVE_DCHECK_GT(packet_capacity,
                static_cast<int32_t>(middle_packet->headers_size()));
  AVE_DCHECK_GT(packet_capacity,
                static_cast<int32_t>(last_packet->headers_size()));
  RtpPacketizer::PayloadSizeLimits limits;
  limits.max_payload_len = packet_capacity - middle_packet->headers_size();

  AVE_DCHECK_GE(single_packet->headers_size(), middle_packet->headers_size());
  limits.single_packet_reduction_len =
      single_packet->headers_size() - middle_packet->headers_size();

  AVE_DCHECK_GE(first_packet->headers_size(), middle_packet->headers_size());
  limits.first_packet_reduction_len =
      first_packet->headers_size() - middle_packet->headers_size();

  AVE_DCHECK_GE(last_packet->headers_size(), middle_packet->headers_size());
  ...
  for (size_t i = 0; i < num_packets; ++i) {
    ...
    int32_t expected_payload_capacity = 0;
    ...
    AVE_DCHECK_LE(packet->payload_size(),
                  static_cast<size_t>(expected_payload_capacity));
...
uint8_t RTPSenderVideo::GetTemporalId(const RTPVideoHeader& header) {
  if (const auto* vp8 =
          std::get_if<RTPVideoHeaderVP8>(&header.video_type_header)) {
    return vp8->temporalIdx;
  }
  if (const auto* vp9 =
          std::get_if<RTPVideoHeaderVP9>(&header.video_type_header)) {
...
bool RTPSenderVideo::UpdateConditionalRetransmit(
  ...
  if (temporal_id != kNoTemporalIdx && temporal_id > 0) {
    ...
    for (int32_t i = temporal_id - 1; i >= 0; --i) {
