/*
 * rtp_video_layers_allocation_extension.cc - RTP video layers allocation
 * extension
 *
...
SpatialLayersBitmasks SpatialLayersBitmasksPerRtpStream(
  ...
  AVE_DCHECK(AllocationIsValid(allocation));
...
bool RtpVideoLayersAllocationExtension::Write(
    ...
    const VideoLayersAllocation& allocation) {
  AVE_DCHECK(AllocationIsValid(allocation));
  AVE_DCHECK_GE(data.size(), ValueSize(allocation));
  ...
  *write_at |= slb.max_rtp_stream_id << 4;
  ...
  AVE_DCHECK_EQ(write_at - data.data(),
                static_cast<ptrdiff_t>(ValueSize(allocation)));
  return true;
}
