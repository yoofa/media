...
int main(int argc, char** argv) {
  ...
  while (file.read(reinterpret_cast<char*>(packet), kTSPacketSize)) {
    ...
    if (err != OK) {
      AVE_LOG(LS_ERROR) << "Failed to parse TS packet " << packet_count
                        << ": error=" << err;
      continue;
    }

    if (event.HasReturnedData()) {
      AVE_LOG(LS_INFO) << "Sync event at packet " << packet_count
    ...
    if (packet_count % 100 == 0) {
      ...
      if (video_source) {
        ...
        while (video_source->Read(frame, nullptr) == OK) {
          ++video_frames;
          AVE_LOG(LS_VERBOSE) << "Got video frame: size=" << frame->size()
                              << ", pts=" << frame->pts().us();
        }
      }
      ...
      if (audio_source) {
        ...
        while (audio_source->Read(frame, nullptr) == OK) {
          ++audio_frames;
          AVE_LOG(LS_VERBOSE) << "Got audio frame: size=" << frame->size()
                              << ", pts=" << frame->pts().us();
        }
      }
    }
  ...
  std::cout << "Has video source: "
            << (parser->HasSource(TSParser::VIDEO) ? "Yes" : "No") << std::endl;
  std::cout << "Has audio source: "
            << (parser->HasSource(TSParser::AUDIO) ? "Yes" : "No") << std::endl;
  ...
  if (audio_source) {
    ...
    if (format) {
      ...
      std::cout << "  Sample Rate: " << format->sample_rate() << " Hz"
                << std::endl;
      std::cout << "  Channels: " << static_cast<int>(format->channel_layout())
                << std::endl;
    }
  }
