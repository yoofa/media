...
**Changed:**
- Reused the repository's existing `foundation/bit_reader.h`
- Updated logging to the repository's `AVE_LOG(...)` style via `base/logging.h`
- Removed assumptions about `base::BitReader` and `LOG()` that do not match this tree

**Benefits:**
- Eliminates mismatches against the current tree
- Reuses existing media foundation utilities
- Consistent with repository logging and parser conventions
...
**TSParser:**
- Uses `ave::media::BitReader` throughout
...
### Before Optimization:
- Assuming a non-existent `base::BitReader`
- Buffer with meta() returning Message
- Mixed Buffer/MediaFrame usage
- Inconsistent metadata handling

### After Optimization:
- Using `foundation/bit_reader.h` from the current repository
