...
enum class DiscontinuityType : int32_t {
  NONE = 0,
  TIME = 1,
  AUDIO_FORMAT = 2,
  VIDEO_FORMAT = 4,
  ...
  FORMATCHANGE = static_cast<int32_t>(AUDIO_FORMAT) |
                 static_cast<int32_t>(VIDEO_FORMAT) |
                 static_cast<int32_t>(TIME),
  FORMAT_ONLY =
      static_cast<int32_t>(AUDIO_FORMAT) | static_cast<int32_t>(VIDEO_FORMAT),
};
...
class PacketSource : public MediaSource {
 ...
 private:
  struct DiscontinuitySegment {
    ...
    DiscontinuitySegment() : max_deque_time_us_(-1), max_enque_time_us_(-1) {}
  ...
  struct QueueEntry {
    ...
    DiscontinuityType discontinuity_type_ = DiscontinuityType::NONE;
    std::shared_ptr<Message> extra_;

    bool IsDiscontinuity() const {
      return discontinuity_type_ != DiscontinuityType::NONE;
    }
  };
