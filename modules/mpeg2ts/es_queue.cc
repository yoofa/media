...
std::shared_ptr<MediaFrame> ESQueue::DequeueAccessUnitH264() {
  ...
  if (!found_start) {
    if (eos_reached_) {
      ...
      auto access_unit = CreateFrameCopy(data, size, MediaType::VIDEO);
  ...
  auto access_unit = CreateFrameCopy(data, au_size, MediaType::VIDEO);
...
std::shared_ptr<MediaFrame> ESQueue::DequeueAccessUnitAAC() {
  ...
  if (eos_reached_ && size > 0) {
    // Return whatever we have
    auto access_unit = CreateFrameCopy(data, size, MediaType::AUDIO);
    int64_t time_us = FetchTimestamp(size);
    if (time_us >= 0) {
      access_unit->SetPts(base::Timestamp::Micros(time_us));
    }
