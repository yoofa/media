...
void OpusCodec::ProcessInput(size_t index) {
...

  {
    ...
    if (is_encoder_) {
      ...
      if (input_buffers_[index].in_use) {
        ...
        if (output_index == kInvalidIndex) {
          ...
          int encoded_bytes = opus_encode(
              opus_ctx_.encoder, pcm, frame_size_, output_buffer->data(),
              static_cast<opus_int32>(output_buffer->capacity()));

          if (encoded_bytes < 0) {
            AVE_LOG(LS_ERROR) << "Opus encoding failed: " << encoded_bytes;
            input_buffers_[index].in_use = false;
            error_to_notify = UNKNOWN_ERROR;
          } else {
            output_buffer->SetRange(0, encoded_bytes);
            AVE_LOG(LS_VERBOSE) << "Encoded " << pcm_samples << " samples to "
                                << encoded_bytes << " bytes";
            pushed_index = PushOutputBuffer(output_index);
          }
        }
      }
    } else {
      ...
      if (output_index == kInvalidIndex) {
        ...
        if (decoded_samples < 0) {
          ...
          if (!is_plc) {
            AVE_LOG(LS_VERBOSE)
                << "Decoded " << opus_size << " bytes to " << decoded_samples
                << " samples (" << pcm_bytes << " bytes)";
          }
          pushed_index = PushOutputBuffer(output_index);
        }
      }
  ...
  if (error_to_notify != OK) {
    NotifyError(error_to_notify);
  }
  if (pushed_index != kInvalidIndex) {
    NotifyOutputBufferAvailable(pushed_index);
  }
  if (should_notify_input) {
