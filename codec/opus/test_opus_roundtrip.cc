...
int main() {
  ...
  for (int i = 0; i < SAMPLE_RATE; i++) {
    opus_int16 sample =
        (opus_int16)(16000 * std::sin(2 * M_PI * 440 * i / SAMPLE_RATE));
  ...
  for (int frame_num = 0; frame_num < num_frames; frame_num++) {
    const opus_int16* frame_data =
        pcm_data.data() + frame_num * FRAME_SIZE * CHANNELS;
    int encoded_bytes =
        opus_encode(encoder, frame_data, FRAME_SIZE, encoded_packet.data(),
                    encoded_packet.size());
    ...
    out_encoded.write(reinterpret_cast<const char*>(encoded_packet.data()),
                      encoded_bytes);

    std::cout << "Encoded frame " << frame_num << ": " << encoded_bytes
              << " bytes" << std::endl;
  }
  ...
  std::cout << "Created /tmp/test_opus_raw.opus with " << num_frames
            << " frames" << std::endl;
  std::cout << "Format: Each frame has 4-byte size header + opus packet data"
            << std::endl;
