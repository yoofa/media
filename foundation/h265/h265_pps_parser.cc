...
std::optional<H265PpsParser::PpsState> H265PpsParser::ParsePps(
  ...
  return ParseInternal(std::span<const uint8_t>(rbsp.data(), rbsp.size()), sps);
}
...
std::optional<H265PpsParser::PpsState> H265PpsParser::ParseInternal(
  ...
  pps.qp_bd_offset_y = static_cast<int32_t>(6 * sps->bit_depth_luma_minus8);
  ...
  if (tiles_enabled_flag) {
    ...
    if (!uniform_spacing_flag) {
      std::array<int32_t, kMaxNumTileRowHeight> column_width_minus1{};
      column_width_minus1[num_tile_columns_minus1] =
          static_cast<int32_t>(sps->pic_width_in_ctbs_y - 1);
      ...
      std::array<int32_t, kMaxNumTileRowHeight> row_height_minus1{};

      row_height_minus1[num_tile_rows_minus1] =
          static_cast<int32_t>(sps->pic_height_in_ctbs_y - 1);
...
bool H265PpsParser::ParsePpsIdsInternal(base::BitstreamReader& reader,
                                        uint32_t& pps_id,
                                        uint32_t& sps_id) {
