...
#include "media/foundation/h265/h265_bitstream_parser.h"
#include <span>
...
H265BitstreamParser::Result H265BitstreamParser::ParseNonParameterSetNalu(
  ...
  if (dependent_slice_segment_flag == 0) {
    ...
    if (slice_type == H265::SliceType::kP ||
      ...
      if (!slice_reader.Ok() ||
        ...
        // pred_weight_table()
        AVE_LOG(LS_ERROR) << "Streams with pred_weight_table unsupported.";
  ...
  if (!slice_reader.Ok() || (abs(last_slice_qp_delta) > kMaxAbsQpDeltaValue)) {
    ...
    AVE_LOG(LS_ERROR) << "Parsed QP value out of range.";
    return kInvalidStream;
  }
  // 7-54 in H265 spec.
...
const H265PpsParser::PpsState* H265BitstreamParser::GetPPS(uint32_t id) const {
  ...
  if (it == pps_.end()) {
    AVE_LOG(LS_WARNING) << "Requested a nonexistent PPS id " << id;
    return nullptr;
  }
...
const H265SpsParser::SpsState* H265BitstreamParser::GetSPS(uint32_t id) const {
  ...
  if (it == sps_.end()) {
    AVE_LOG(LS_WARNING) << "Requested a nonexistent SPS id " << id;
    return nullptr;
  }
...
void H265BitstreamParser::ParseSlice(std::span<const uint8_t> slice) {
  if (slice.empty()) {
    AVE_LOG(LS_WARNING) << "Empty slice in H265 bitstream.";
    return;
  }
  ...
  switch (nalu_type) {
    case H265::NaluType::kVps: {
      ...
      if (!vps_state) {
        AVE_LOG(LS_WARNING) << "Unable to parse VPS from H265 bitstream.";
      } else {
    ...
    case H265::NaluType::kSps: {
      ...
      if (!sps_state) {
        AVE_LOG(LS_WARNING) << "Unable to parse SPS from H265 bitstream.";
      } else {
    ...
    case H265::NaluType::kPps: {
      ...
      if (slice.size() >= H265::kNaluHeaderSize) {
        ...
        base::BitstreamReader slice_reader(std::span<const uint8_t>(
            unpacked_buffer.data(), unpacked_buffer.size()));
      ...
      if (!pps_state) {
        AVE_LOG(LS_WARNING) << "Unable to parse PPS from H265 bitstream.";
      } else {
    ...
    default:
      ...
      if (res != kOk) {
        AVE_LOG(LS_INFO) << "Failed to parse bitstream. Error: " << res;
      }
      break;
  }
}
...
H265BitstreamParser::ParsePpsIdFromSliceSegmentLayerRbsp(
  ...
  base::BitstreamReader slice_reader(
      std::span<const uint8_t>(unpacked_buffer.data(), unpacked_buffer.size()));
...
std::optional<uint32_t> H265BitstreamParser::GetLastSlicePpsId() const {
  if (!last_slice_pps_id_) {
    AVE_LOG(LS_ERROR) << "Failed to parse PPS id from bitstream.";
    return std::nullopt;
  }
